...
namespace LearnHub.Back.Api.Controllers
{
    ...
    public class CourseController : ControllerBase
    {
        ...
        /// <summary>
        /// Gets top 10 courses by demand
        /// </summary>
        /// <returns>List of top demanded courses</returns>
        [HttpGet("top-demand")]
        [SwaggerOperation(
            Summary = "Gets the top 10 courses by enrollment count",
            Description = "Retrieves the 10 courses with the highest number of enrollments")]
        [ProducesResponseType(typeof(List<CourseDto>), StatusCodes.Status200OK)]
        public async Task<ActionResult<List<CourseDto>>> GetTopByDemand()
        {
            var result = await _mediator.Send(new GetTopCoursesByDemandQuery());
            return Ok(result);
        }
